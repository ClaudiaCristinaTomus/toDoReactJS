...
const App = () =>{
  ...
  const deleteTdoHandler=useCallback(
    (id)=>{
      const oldTodos = [...todos];
  ...
  const savedEditTodoHandler =useCallback((id)=>{
    setEditing(false);
    setTodoIndex(null);
            ...
            <div key={todo.id} className="todos-container-child">
              {editing && todoIndex === index ? (
                <div>
                  ...
                  />
                  <button onClick={()=>savedEditTodoHandler(todo.id)}>Save</button>
                </div>
              ):<>
               <div>
              ...
              <div>
                <MdDelete 
                  className="icon" 
                  onClick={()=>deleteTdoHandler(todo.id)}
                />
                <MdEdit 
                  className="icon" 
